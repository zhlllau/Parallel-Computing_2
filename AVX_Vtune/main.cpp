...
float** SSE_gaosi_part1(float **A, int n){
    ...
    for (int k=0;k<n;k++){
      ...
      //将第k行第k个元素全部变为1
      float tmp[8] = {A[k][k], A[k][k], A[k][k], A[k][k], A[k][k],A[k][k],A[k][k],A[k][k]}; //创建一个浮点数向量A[k][k]
      t1 = _mm256_loadu_ps(tmp);  //将向量float装载进t1中 {*不对齐形式}
      ...
      for(j=n-8;j>=k;j = j-8)
           {
            ...
            t2 = _mm256_loadu_ps(A[k]+j);   //将第k行的j~j+4列元素装载进来
            t3 = _mm256_div_ps(t2, t1); //向量除
            _mm256_storeu_ps(A[k]+j, t3);   //将向量结果装载回去
